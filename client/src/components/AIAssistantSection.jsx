...
const AIAssistantSection = () => {
  ...
  // Fungsi untuk trigger klik pada input file yang disembunyikan
  ...
  // Fungsi untuk menghapus file yang batal diupload
  const handleRemoveFile = () => {
    setSelectedFile(null);
    if (fileInputRef.current) {
      fileInputRef.current.value = ""; // Reset value input
    }
  };

  // Simulasi tombol kirim
  const handleSend = () => {
    if (!promptText && !selectedFile) return;
    console.log("Mengirim ke AI:", { text: promptText, file: selectedFile });
    alert("Cek Console (F12) untuk melihat data yang dikirim!");
    // Nanti logika nembak API/Backend AI taruh di sini ya Bang
  };
  ...
  return (
    <section className="relative w-full min-h-screen flex flex-col items-center justify-center bg-bianca-50 overflow-hidden py-24 px-4 sm:px-6 shadow-[inset_0_10px_20px_rgba(0,0,0,0.03)]">
      ...
      {/* --- EFEK BLOB (WARNA COKELAT ESTETIK) --- */}
      ...
      <div
        data-aos="fade-right"
        data-aos-delay="200"
        className="hidden md:block absolute left-[-5%] lg:left-[2%] top-1/2 transform -translate-y-1/2 w-48 lg:w-72 opacity-90 z-10 pointer-events-none transition-all duration-300"
      >
        <img
          src={barong} 
          alt="Ornamen Barong Kiri" 
          className="hidden md:block absolute left-[-5%] lg:left-[2%] top-1/2 transform -translate-y-1/2 w-48 lg:w-72 opacity-90 z-10 pointer-events-none transition-all duration-300"
        />
      </div>
      <div
        data-aos="fade-left"
        data-aos-delay="200"
        className="hidden md:block absolute right-[-5%] lg:right-[2%] top-1/2 transform -translate-y-1/2 w-48 lg:w-72 opacity-90 z-10 pointer-events-none transition-all duration-300"
      >
        <img 
          src={barong} 
          alt="Ornamen Barong Kanan" 
          className="hidden md:block absolute right-[-5%] lg:right-[2%] top-1/2 transform -translate-y-1/2 w-48 lg:w-72 opacity-90 z-10 pointer-events-none transition-all duration-300"
        />
      </div>
      ...
      <div className="relative z-20 w-full max-w-4xl mx-auto text-center flex flex-col items-center">
        ...
        <div data-aos="fade-down" className="flex items-center bg-white w-full max-w-3xl min-h-[60px] md:min-h-[75px] rounded-full shadow-[0_20px_50px_rgba(0,0,0,0.08)] px-3 md:px-4 border border-gray-100 relative z-30 transition-all duration-300 hover:shadow-[0_25px_60px_rgba(90,68,46,0.1)] focus-within:shadow-[0_25px_60px_rgba(90,68,46,0.1)] py-2">
          ...
          <button 
            ...
            className="p-2 md:p-3 text-gray-400 hover:text-inv-base transition cursor-pointer flex-shrink-0"
            title="Unggah Gambar"
          >
            <FaPaperclip size={18} className="md:text-xl" />
          </button>
          
          {/* Container Input Teks & Indikator File */}
          <div className="flex-1 flex flex-wrap items-center gap-2 overflow-hidden px-2">
            
            {/* Indikator File (Muncul kalau ada file yang dipilih) */}
            {selectedFile && (
            ...
            <input 
              ...
              placeholder={selectedFile ? "Tambahkan pesan..." : "Cari tahu tentang Budaya Indonesia disini..."} 
              className="flex-1 min-w-[150px] bg-transparent border-none outline-none text-inv-base font-teachers py-2 text-sm md:text-lg placeholder-gray-400"
            />
          </div>
          ...
          <button 
            onClick={handleSend}
            className="bg-inv-base text-white font-bold font-teachers text-sm md:text-base px-6 md:px-10 h-[45px] md:h-[55px] rounded-full hover:bg-roman-coffee-800 transition shadow-md ml-2 cursor-pointer flex-shrink-0"
          >
            Kirim
          </button>
