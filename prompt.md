...
baiklah lanjutkan, perlu anda ketahui bahwa kesepakatan saya dan tim saya sepakat kalo input yang dikirim user akan memiliki 4 sekenario seperti ini
1. user kirim propmt dan image, maka image nya yang akan di teruskan ke HF API dan kita menunggu jawaban dari HF API itu, nanti setelah jawaban di terima maka hasil clasification akan di teruskan ke gemini melalui API biarkan gemini menjawab agar lebih natural. setelah terjawab maka response akan di simpan kedalam tabel ai_chats dimana disini sudah lengkap datanya sesuai apa yang  ada di tabel ai_chats.
2. user kirim propmt tetapi tidak mengirim Image, maka propmt tidak akan diteruskan ke HF API namun akan diteruskan ke gemini API agar bisa mendapatkan jawaban natural, jawaban dari gemini akan di simpan kedalam tabel ai-chats. dan di kirim kembali ke front end.
3. user tidak kirim prompt tetapi user kirim image, maka image akan diteruskan ke HF API kemudian jawaban yang diberikan akan di tersukan ke gemini, dan gemini menjelaskan, jawaban dari gemini akan di simpan kedalam tabel ai-chats. dan di kirim kembali ke front end.
4. user tidak mengirim prompt dan image, maka langsung di tolak di controller.

NOTE: disini gemini hanya bertugas memberikan jawaban natural, dia tidak memproses gambar, dan tidak men-generate gambar baru. tabel tetap mennyimpan link dari image yang dikirm oleh user yang menyimpan image hanyalah cloudinary.